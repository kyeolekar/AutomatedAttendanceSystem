...
class FaceRecognition:
    ...
    main_d = "/home/krishna/Project/Project/project-detect/"
    ...
    def collect_dataset(self):
        ...
        labels = []
        labels_dic = {}
        people = [person for person in os.listdir(self.main_d + "people/")]
        for i, person in enumerate(people):
            labels_dic[i] = person
            for image in os.listdir(self.main_d +"people/" + person):
                images.append(cv2.imread(self.main_d +"people/" + person + "/" + image, 0))
                labels.append(i)
        return images, np.array(labels), labels_dic

    def detect_face(self, frame):
        detector = cv2.CascadeClassifier(self.main_d + 'frontal_face.xml')
    ...
    def train_recognizer(self):
        ...
        rec_eig.save(self.main_d +"faces_trained_lbph.xml")

        images, labels, labels_dic = self.collect_dataset()
        rec_eig = cv2.face.createEigenFaceRecognizer()
        rec_eig.train(images, labels)
        rec_eig.save(self.main_d +"faces_trained_eigen.xml")

        images, labels, labels_dic = self.collect_dataset()
        rec_eig = cv2.face.createFisherFaceRecognizer()
        rec_eig.train(images, labels)
        rec_eig.save(self.main_d +"faces_trained_fisher.xml")
    ...
    def start_recognition(self):
        ...
        images, labels, labels_dic = self.collect_dataset()
        rec_eig = cv2.face.createLBPHFaceRecognizer()
        rec_eig.load(self.main_d +"faces_trained_lbph.xml")

        rec_fisher = cv2.face.createFisherFaceRecognizer()
        rec_fisher.load(self.main_d +"faces_trained_fisher.xml")

        rec_eigen = cv2.face.createEigenFaceRecognizer()
        rec_eigen.load(self.main_d +"faces_trained_eigen.xml")
...
def meth1():
    print("meth1")
    recog = FaceRecognition(0)
    recog.start_recognition()
